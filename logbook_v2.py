...
def update_sheets():
    ...
    runs = [os.path.dirname(str(path))[len(processed_folder)+1:] for path in Path(processed_folder).rglob('CORRECT.LP')] + [os.path.dirname(str(path))[len(processed_folder)+1:].replace('/streams','') for path in Path(processed_folder).rglob('streams/*stream?*') if 'indexamajig.' not in str(path)]
